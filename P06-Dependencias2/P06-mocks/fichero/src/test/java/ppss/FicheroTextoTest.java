...
public class FicheroTextoTest {
    ...
    @Test
    public void contarCaracteres_C2(){
        //preparar datos de entrada
        ...
        //preparar las expectativas
        assertDoesNotThrow(
                () -> EasyMock.expect(mock1.getFichero(nombreFichero)).andReturn(mock2)
        );
        assertDoesNotThrow(
                () -> EasyMock.expect(mock2.read())
                .andReturn((int) 'a')
                .andReturn((int) 'b')
                .andReturn((int) 'c')
                .andReturn(-1)
